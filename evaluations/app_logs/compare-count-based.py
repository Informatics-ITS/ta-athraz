...
def count_similarity(messages1, messages2):
    c1 = Counter(messages1)
    c2 = Counter(messages2)
...
def debug_similarity(log1_lines, log2_lines):
    ...
    debug_output = []
    debug_output.append("Messages in log1 but NOT in log2:")
    for msg in (c1 - c2).elements():
        debug_output.append(f"  - {msg}")

    debug_output.append("\nMessages in log2 but NOT in log1:")
    for msg in (c2 - c1).elements():
        debug_output.append(f"  - {msg}")
...
if __name__ == "__main__":
    ...
    with open("comparison-results-count-based.txt", "w") as result_file, open("debug-output-count-based.txt", "w") as debug_file:
    ...
    print(" - comparison-results-count-based.txt")
    print(" - debug-output-count-based.txt")